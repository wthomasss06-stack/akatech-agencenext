      ...
      <div style={{ display: 'flex', gap: '.6rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        {PLATFORMS.map(p => (
          <button key={p.id} onClick={() => handleClick(p)} title={`Demander à ${p.label}`}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '.5rem', padding: '.75rem .95rem', borderRadius: 14, background: T.light ? 'rgba(0,0,0,.03)' : 'rgba(255,255,255,.04)', border: `1px solid ${copied === p.id ? '#88ca53' : T.border}`, cursor: 'pointer', minWidth: 70, transition: 'all .2s' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = p.hoverColor; e.currentTarget.style.transform = 'translateY(-3px)' }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = copied === p.id ? '#88ca53' : T.border; e.currentTarget.style.transform = 'none' }}>
