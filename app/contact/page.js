...
function HeroContact() {
  const T = useTheme()
  return (
    <section style={{ padding: 'clamp(6rem,12vw,9rem) 5% clamp(3rem,6vw,5rem)', background: T.bg, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: 'min(600px,100vw)', height: 'min(600px,100vw)', borderRadius: '50%', background: 'radial-gradient(circle,rgba(34,200,100,.07),transparent 65%)', pointerEvents: 'none' }} />
      <div className="grid-bg" style={{ position: 'absolute', inset: 0, opacity: .3 }} />
      <LaserBeam position="left" intensity="strong" />
      <LaserBeam position="right" intensity="subtle" style={{ animationDelay: '.8s' }} />
      <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7, ease: [.22, 1, .36, 1] }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.9rem,5vw,3.8rem)', fontWeight: 800, fontFamily: "'Syne',sans-serif", color: T.textMain, letterSpacing: '-.04em', lineHeight: 1.1, marginBottom: '1.2rem' }}>
            Parlons de votre <span className="text-gradient">projet</span>
          </h1>
          <p style={{ fontSize: 'clamp(.9rem,2vw,1.05rem)', color: T.textSub, lineHeight: 1.75, maxWidth: 560, margin: '0 auto 2.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '.6rem', flexWrap: 'wrap' }}>
            {[
              ...
              <motion.div key={label} initial={{ opacity: 0, scale: .8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .3 + i * .1 }}
                ...
                <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 'clamp(.6rem,.9vw,.68rem)', color: T.textSub, letterSpacing: '.06em' }}>{label}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
    ...
    <section ref={ref} style={{ padding: 'clamp(3rem,6vw,5rem) 5% clamp(2rem,4vw,3rem)', background: T.bgAlt }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .5 }}
          ...
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '.65rem', color: T.textMuted, letterSpacing: '.1em', textTransform: 'uppercase' }}>Réseaux sociaux</span>
          <div style={{ display: 'flex', gap: '.6rem' }}>
            {[
              ...
              <a key={label} href={href} target="_blank" rel="noreferrer" title={label}
                style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(34,200,100,.06)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.textSub, transition: 'all .2s' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#22c864'; e.currentTarget.style.color = '#fff'; e.currentTarget.style.transform = 'translateY(-2px)' }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(34,200,100,.06)'; e.currentTarget.style.color = T.textSub; e.currentTarget.style.transform = 'none' }}>
                <Icon size={15} />
              </a>
            ))}
          </div>
...
function ChannelInfo({ label, val, desc, T }) {
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '.58rem', color: T.green, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '.2rem' }}>{label}</div>
