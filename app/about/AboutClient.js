...
function AboutStatsSlide() {
  ...
  return (
    <div style={{
      ...
      {/* compteur */}
      <div>
        <span style={{ fontFamily: "'Space Mono',monospace", fontSize: '.65rem', letterSpacing: '.15em', color: T.textMuted, opacity: .6 }}>
      ...
      <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '.3rem', minHeight: 92, animation: 'aboutStatIn .45s cubic-bezier(.22,1,.36,1)' }}>
        <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 'clamp(2.6rem,4vw,3.4rem)', fontWeight: 800, lineHeight: 1, color: T.textMain, letterSpacing: '-.02em' }}>
          <AboutStatNumber target={cur.target} suffix={cur.suffix} />
        </span>
        <p style={{ fontFamily: "'Syne',sans-serif", fontSize: '.85rem', fontWeight: 700, letterSpacing: '.04em', color: T.textMain, margin: 0 }}>{cur.label}</p>
    ...
    <section style={{ padding: '7rem 5%', background: T.bgAlt, position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="stats-founder-grid">
        ...
        <BlurReveal direction="right" delay={0.15}>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem,3vw,2.6rem)', fontWeight: 800, fontFamily: "'Syne',sans-serif", color: T.textMain, letterSpacing: '-.03em', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem', padding: '1.2rem 1.4rem', borderRadius: 16, background: 'rgba(136,202,83,.06)', border: '1px solid rgba(136,202,83,.2)', marginBottom: '1.8rem' }}>
            <div style={{ width: 60, height: 60, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '2px solid rgba(136,202,83,.5)', boxShadow: '0 0 16px rgba(136,202,83,.2)' }}>
              <LazyImg
                ...
                placeholder={<div style={{ width: 60, height: 60, background: 'rgba(136,202,83,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#88ca53', fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: '1.2rem' }}>E</div>}
              />
            </div>
            <div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '.95rem', color: T.textMain, marginBottom: '.2rem' }}>M'Bollo Aka Elvis</div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '.62rem', color: T.textMuted, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: '.55rem' }}>Développeur Full-Stack · Fondateur, AKATech</div>
              <a href="https://mbolloaka-dev.vercel.app/" target="_blank" rel="noreferrer"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '.35rem', fontFamily: "'Syne',sans-serif", fontSize: '.7rem', fontWeight: 700, color: '#88ca53', textDecoration: 'none', padding: '.25rem .7rem', borderRadius: 100, background: 'rgba(136,202,83,.1)', border: '1px solid rgba(136,202,83,.3)', transition: 'all .2s' }}
            ...
            <BlurReveal key={year} delay={i * 0.15} direction={i % 2 === 0 ? 'left' : 'right'}>
              <div style={{ display: 'flex', justifyContent: i % 2 === 0 ? 'flex-start' : 'flex-end', marginBottom: '3rem', position: 'relative' }}>
                ...
                <motion.div className="sku-card" whileHover={{ y: -4, scale: 1.01 }} style={{ width: '44%', padding: '1.5rem', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <h3 style={{ fontSize: '.95rem', fontWeight: 700, color: T.textMain, fontFamily: "'Syne',sans-serif", marginBottom: '.4rem' }}><LetterReveal text={title} stagger={0.03} /></h3>
...
function ValuesSection() {
  ...
  return (
    <section style={{ padding: '7rem 5%', background: T.bgAlt, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', right: '-2%', top: '50%', transform: 'translateY(-50%)', fontFamily: "'Syne',sans-serif", fontSize: 'clamp(10rem,18vw,18rem)', fontWeight: 900, color: T.light ? 'rgba(136,202,83,.04)' : 'rgba(136,202,83,.03)', lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>VALUES</div>
          ...
          {VALUES.map(({ icon: Icon, title, desc }, i) => (
            <BlurReveal key={title} delay={i * 0.1} direction={dirs[i % dirs.length]}>
              <motion.div className="sku-card" whileHover={{ y: -5 }} style={{ padding: '2rem', height: '100%', position: 'relative', overflow: 'hidden' }}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: T.textMain, fontFamily: "'Syne',sans-serif", marginBottom: '.5rem' }}><LetterReveal text={title} stagger={0.025} /></h3>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.5rem' }}>
            {SKILLS.map((s, i) => (
              <motion.span key={s} initial={{ opacity: 0, y: 8 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .3 + i * .04 }}
                ...
                style={{ padding: '.35rem .85rem', background: 'rgba(136,202,83,.07)', border: `1px solid ${T.border}`, borderRadius: 100, fontFamily: "'Syne',sans-serif", fontSize: '.65rem', fontWeight: 600, color: T.green, letterSpacing: '.06em', cursor: 'default', transition: 'all .2s' }}>
                {s}
              </motion.span>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
          <BlurReveal direction="left">
            ...
            <h2 style={{ fontSize: 'clamp(1.9rem,3.5vw,2.6rem)', fontWeight: 800, fontFamily: "'Syne',sans-serif", color: T.textMain, letterSpacing: '-.03em', lineHeight: 1.2, marginBottom: '1.2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap' }}>
              {['100% remote', 'WhatsApp & Zoom', 'Suivi en temps réel', 'FCFA & EUR'].map(b => (
                <span key={b} style={{ padding: '.3rem .85rem', borderRadius: 100, background: 'rgba(136,202,83,.08)', border: '1px solid rgba(136,202,83,.2)', fontFamily: "'Syne',sans-serif", fontSize: '.7rem', fontWeight: 600, color: '#88ca53' }}>{b}</span>
              ))}
            </div>
          </BlurReveal>
          <BlurReveal direction="right" delay={0.15}>
...
export default function AboutPage() {
  ...
  return (
    <div>
      ...
      {/* CTA unique */}
      <section ref={ref} style={{ padding: '7rem 5%', background: T.bg, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh' }}>
        <div className="grid-bg" style={{ position: 'absolute', inset: 0, opacity: T.light ? .3 : .15 }} />
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 700, height: 400, borderRadius: '50%', background: 'radial-gradient(ellipse,rgba(136,202,83,.1),transparent 65%)', pointerEvents: 'none' }} />
        <motion.div initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }} animate={inView ? { opacity: 1, y: 0, filter: 'blur(0px)' } : {}} transition={{ duration: .8, ease: [.22,1,.36,1] }}
          ...
          <motion.div initial={{ opacity: 0, scale: .9 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: .1 }}
            ...
            <span style={{ fontFamily: "'Syne',sans-serif", fontSize: '.65rem', fontWeight: 700, color: '#88ca53', letterSpacing: '.1em', textTransform: 'uppercase' }}>Prêt à collaborer ?</span>
          </motion.div>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .15 }}
            style={{ fontSize: 'clamp(1.5rem,3.2vw,2.4rem)', fontWeight: 800, fontFamily: "'Syne',sans-serif", color: T.textMain, letterSpacing: '-.03em', lineHeight: 1.3, marginBottom: '2rem' }}>
            Prêt à collaborer avec AKATech ?<br />Discutons de votre projet dès maintenant.
          </motion.p>
